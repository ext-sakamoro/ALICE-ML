...
use alice_ml::{
    quantize_to_ternary, ternary_matvec, ternary_matvec_kernel, TernaryWeight, TernaryWeightKernel,
};
...
fn bench_ternary_matvec_dps(c: &mut Criterion) {
    ...
    for size in &[64, 256, 512, 1024] {
        ...
        let weight_data: Vec<i8> = (0..*size * *size).map(|i| (i % 3) as i8 - 1).collect();
...
fn bench_ternary_kernel_dps(c: &mut Criterion) {
    ...
    for size in &[64, 256, 512, 1024] {
        ...
        group.bench_with_input(BenchmarkId::new("bitparallel", size), size, |b, _| {
            b.iter(|| {
...
fn bench_quantization(c: &mut Criterion) {
    ...
    for size in &[256, 512, 1024] {
        ...
        group.bench_with_input(BenchmarkId::new("size", size), size, |b, _| {
            b.iter(|| black_box(quantize_to_ternary(&weights, *size, *size)));
        });
    }
